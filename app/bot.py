...
async def create_db_connections(dp: Dispatcher) -> Tuple[AsyncIOMotorClient, Cache]:
    logger: structlog.typing.FilteringBoundLogger = dp["business_logger"]

    logger.debug("DataBase Initialization")

    mongo_client = AsyncIOMotorClient(str(settings.MONGODB_URI), server_api=ServerApi('1'))
    dp["mongo_client"] = mongo_client
...
async def close_db_connections(dp: Dispatcher) -> None:
    ...
    if "mongo_client" in dp.workflow_data:
        mongo_client: AsyncIOMotorClient = dp["mongo_client"]
        mongo_client.close()
...
def setup_handlers(dp: Dispatcher) -> None:
    dp.include_router(handlers.json.router)


def setup_middlewares(dp: Dispatcher, mongo_db: AsyncIOMotorDatabase, cache: Cache) -> None:
    dp.update.outer_middleware(StructLoggingMiddleware(logger=dp["aiogram_logger"]))
    # dp.update.middleware(DBSessionMiddleware(session_pool=async_session))
    dp.update.middleware(DatabaseMiddleware(cache=cache, mongo_db=mongo_db))
...
async def setup_aiogram(dp: Dispatcher) -> None:
    ...
    mongo_client, cache = await create_db_connections(dp)

    setup_handlers(dp)
    setup_middlewares(dp, cache=cache, mongo_db=mongo_client[settings.MONGODB_DATABASE])
